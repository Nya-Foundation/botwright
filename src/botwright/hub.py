...
class MessageHubStats:
    ...
    message_count: int = 0
    matched_count: int = 0
    recent_gateway_events: deque[str] = field(default_factory=lambda: deque(maxlen=20))
...
class MessageWaiter:
    ...
    observed: deque[ObservedMessage] = field(default_factory=lambda: deque(maxlen=50))
...
class MessageHub:
    def __init__(self) -> None:
        ...
        self._any_channel_waiters: list[MessageWaiter] = []
    ...
    def register(
        ...
        if channel_id is None:
            self._any_channel_waiters.append(waiter)
        else:
            self._waiters[channel_id].append(waiter)
        return waiter

    def unregister(self, waiter: MessageWaiter) -> None:
        if waiter.channel_id is None:
            with suppress(ValueError):
                self._any_channel_waiters.remove(waiter)
            return
    ...
    def dispatch(self, message: discord.Message) -> None:
        ...
        waiters = [
            *self._any_channel_waiters,
            *self._waiters.get(message.channel.id, ()),
        ]
